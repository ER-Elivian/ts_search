...
def move_bond_3(rpath,xyzs_strs,num_A, num_center, num_B, lens, not_optimized_in_iteration,init_bonds, grad_strs, control_strs, logname): #SN2 реакция
    ...
    if  g_c_p_len/g_c_len<0.1 or g_c_p_len<0.00001:
        ...
        res_Ac=Ac_len+g_A_p_sign*change_fn(g_A_p_len, 0.05)
        res_Bc=Bc_len+g_B_p_sign*change_fn(g_B_p_len, 0.05)
        ...
        change=change_fn(g_c_p_len,0.03)
...
def move_bond_2(rpath,xyzs_strs, num_A, num_B, lens, not_optimized_in_iteration,init_bonds, grad_strs, control_strs, logname):
    ...
    bond_change=s_g_p_sign*change_fn(proj_len,0.01)
    ...
    #Пояснение- если в результате поиска ПС связь порвалась или замкнулась, то  результат сбрасывается, а в начальной геометрии соответствующая связь немного (на 0,02) растягивается или укорачивается, чтобы притяжение или отталкивание было меньше
        not_optimized_in_iteration[rpath]=True
        ...
        while line != "":
            if "SN2" in line:
                ...
                while line.startswith("    b"):
                    ...
                    bonds_sn2.append([int(line_split[1]), int(line_split[2])])
                    line=bonds.readline()
                
                if len(bonds_sn2)!=2:
                    print("not two bonds in Sn2 section")
                else:
                    if bonds_sn2[0][0]==bonds_sn2[1][0]:
                        to_append=[bonds_sn2[0][1],bonds_sn2[0][0],bonds_sn2[1][1]]
                    elif bonds_sn2[0][0]==bonds_sn2[1][1]:
                        to_append=[bonds_sn2[0][1],bonds_sn2[0][0],bonds_sn2[1][0]]
                    elif bonds_sn2[0][1]==bonds_sn2[1][0]:
                        to_append=[bonds_sn2[0][0],bonds_sn2[0][1],bonds_sn2[1][1]]
                    else:
                        to_append=[bonds_sn2[0][0],bonds_sn2[0][1],bonds_sn2[1][0]]
                    search_bonds[rpath].append(to_append)
            elif line.startswith("b"):#все остальные реакции обозначим просто связями без отступа
                print(line)
                line_split=line.split()
                search_bonds[rpath].append([int(line_split[1]), int(line_split[2])])
            line=bonds.readline()
    return chrg,solvent
...
path=os.path.join(initial_cwd,"medv_art")
os.chdir(path)
print(path)
...
while not_completed:
    ...
    if not_optimized_in_iteration[rpath]:
        ...
        with open(os.path.join(rpath,"xtbout"),"w+") as xtbout:
            subprocess.call(["xtb", xyz_name, "-I", "control","--alpb",solvent,"--opt","--tight"],stdout=xtbout)
        ...
        for bond in search_bonds[rpath]:
            len_bond=len(bond)
            if len_bond == 2:
                proj_len=move_bond_2(rpath,xyzs_strs,bond[0],bond[1],lens, not_optimized_in_iteration, init_bonds, grad_strs,control_strs,logname)
            elif len_bond == 3:
        ...
        os.chdir(rpath)
        with open(os.path.join(rpath,"xtbout"),"w+") as xtbout:
            subprocess.call(["xtb", "xtbopt.xyz", "-I", "control", "--alpb",solvent,"--opt","--tight"],stdout=xtbout)
