...
prev_dc=100
global change_mode
global preferred_change_mode
preferred_change_mode=0
...
def opt(rpath,xyz_name,solvent):
    with open(os.path.join(rpath,"xtbout"),"w+") as xtbout:
        if solvent=="vacuum":
            subprocess.call(["xtb", xyz_name, "-I", "control","--opt","--tight"],stdout=xtbout)
        else:
...
def move_bonds(rpath, xyzs_strs, search_bonds, lens, not_optimized_in_iteration, init_bonds, grad_strs, control_strs, logname):
    ...
    if DoC_cutoff==0:
        DoC_cutoff=div_of_changes
    if div_of_changes<DoC_cutoff: # нужно "отпущение" связей
        print("\nstructure laying on reaction path found")
    ...
    change_if_less_DoC=preferred_change_mode*(change_fn(mean_change,0.01))
    ...
    for bond in search_bonds:
        ...
        if div_of_changes<DoC_cutoff: # нужно "отпущение" связей
            ...
            change_mode=-preferred_change_mode
            pass_turns=2#1 снимется уже в конце этого хода
        elif div_of_changes<prev_dc or pass_turns>0:
            print(f"{div_of_changes<prev_dc} {change_mode}")
            bond_change=change_mode*(change_fn(changes[key]-mean_change,0.01))
        else:
            cur_c_m=-change_mode
    ...
    prev_dc=div_of_changes
    if change_mode*cur_c_m<0:
        change_mode=cur_c_m
...
path=os.path.join(initial_cwd,"tests","apw3_test")
os.chdir(path)
print(path) 
...
phases={}#["a, b"] фазы. Связи с одними фазами растягиваются, с разными - сжимаются
...
if reac_type==2:    
    for node in Reag_graph.nodes():
        if len(Reag_graph.neighbors(node))!=1:
            print("strange reaction")
            exit()
    preferred_change_mode=1
elif reac_type==1:    
    for node in Reag_graph.nodes():
        connected_nodes=Reag_graph.neighbors(node)
